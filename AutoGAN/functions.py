...
def train(
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        if global_steps % args.n_critic == 0:
            ...
            if args.regan and gen_net.train_on_sparse:
                gen_net.apply_masks()
                for mask_w, ema_w in zip(gen_net.masks, gen_avg_param):
                    ema_w.data[mask_w] = 0
...
def get_is(args, gen_net: nn.Module, num_img):
    ...
    for ii in range(eval_iter):
        ...
        for iii in range(args.eval_batch_size):
            utils.save_image(gen_imgs[iii].detach(), '%s/f_%s.png' % (evaroot, str(iii + ii * 50)),
                             normalize=True, range=(-1, 1))

    # get inception score
    logger.info("calculate Inception score...")

    dataset = ImageDataset(evaroot, exts=['png', 'jpg'])
    loader = torch.utils.data.DataLoader(dataset, batch_size=50, num_workers=4)
...
def validate(args, fixed_z, fid_stat, gen_net: nn.Module, writer_dict, clean_dir=True):
    ...
    img_list = list()
    for ii in tqdm(range(eval_iter), desc="sample images"):
        z = torch.cuda.FloatTensor(
            np.random.normal(0, 1, (args.eval_batch_size, args.latent_dim))
        )
        ...
        gen_imgs = gen_net(z)
        for iii in range(args.eval_batch_size):
            utils.save_image(gen_imgs[iii].detach(), '%s/f_%s.png' % (evaroot, str(iii + ii * args.eval_batch_size)),
                             normalize=True, range=(-1, 1))
    ...
    fid_score = fid.calculate_fid_given_paths(['./fid_stat/cifar10.test.npz', evaroot], 100, device, 2048)
